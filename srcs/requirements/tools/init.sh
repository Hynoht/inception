...
check_data_() {
	if [ "$(ls -A $HOME/data 2>/dev/null)" = "" ]; then
		echo "Le dossier data est vide, Veuillez rebuilt le programme"
		exit 1
	fi
	if [ "$(ls -A $HOME/data/mariadb 2>/dev/null)" = "" ]; then
		echo "Le dossier data est vide, Veuillez rebuilt le programme"
		exit 1
	fi
	if [ "$(ls -A $HOME/data/wordpress 2>/dev/null)" = "" ]; then
		echo "Le dossier data est vide, Veuillez rebuilt le programme"
		exit 1
	fi
	if [ "$(ls -A $HOME/data/phpmyadmin 2>/dev/null)" = "" ]; then
		echo "Le dossier data est vide, Veuillez rebuilt le programme"
		exit 1
	fi
}
